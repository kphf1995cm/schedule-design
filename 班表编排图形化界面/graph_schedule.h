...
public:
	...
	bool TestError()//检查输入数据是否正确
	{
		...
		//int Row, Column;
		//int StudentNum = 0;
		infile.open("D:\\SpareTimeTable.txt", ios::in);
		if (!infile)
		{
	...
	void WriteManyPoint(int _num,fstream& _outfile)
	{
		while (_num)
		{
			_outfile.put('.');
	...
	void WriteManyShortLine(int _num, fstream& _outfile)
	{
		while (_num)
		{
			_outfile.put('-');
	...
	void WriteDay_7(int _num, fstream& _outfile)
	{
		while (_num)
		{
			_outfile.put('*');
		...
		_outfile.put('|');
		const char day1[10] = { " 星期一 |" };
		for (int i = 0; i < 10; i++)
			_outfile.put(day1[i]);
		const char day_7[6][9] = { "星期二 |", "星期三 |", "星期四 |", "星期五 |", "星期六 |", "星期日 |" };
		for (int i = 0; i < 6; i++)
		{
			for (int j = 0; j < 9; j++)
				_outfile.put(day_7[i][j]);
		}
		_outfile.put('\n');
	}

	void WriteXing(int _num, fstream& _outfile)
	{
		while (_num)
		{
			_outfile.put('*');
	...
	void WriteSchedule(char **_time,int length,CString filename)
	{
		fstream outfile;
		//CString filename("D:\\Aschedule.txt");
		...
		int count = 0;
		//cout << "..........................................................................." << endl;
		WriteManyPoint(100, outfile);
		WriteDay_7(11, outfile);
		WriteManyShortLine(100, outfile);
		//cout << "***********|" << " 星期一 |" << " 星期二 |" << " 星期三 |" << " 星期四 |" << " 星期五 |";
		//cout << " 星期六 |" << " 星期日 |" << endl;
		//cout << "---------------------------------------------------------------------------" << endl;
		int i = 0;
				...
				while (j < mDayCount)
				{
					if (j == 0 && k == 0)
					{
						for (int p = 0; p<length; p++)
							outfile.put(_time[count][p]);
						count++;
					}
					if (j == 0 && k != 0)
						WriteXing(11, outfile);
					if (mArrangePersonMatrix[i][j].size()>k)
					{
						int size = mArrangePersonMatrix[i][j][k]->mName.size();
						if (size == 4) //cout << "**";
						{
							outfile.put('*'); outfile.put('*');
						}
						if (size == 6) //cout << "*";
							outfile.put('*');
						for (int t = 0; t < size; t++)
						{
							//cout << mArrangePersonMatrix[i][j][k]->mName[t];
							outfile.put(mArrangePersonMatrix[i][j][k]->mName[t]);
						}
						if (size == 4)
						{
							//cout << "**|";
				...
				while (point<mSPersonneed)
				{
					for (int c = 0; c < mDayCount; c++)
					{
						...
						else
						{
							...
							{
								//cout << (*mSpecailPerson)[point]->mName[t];
								outfile.put((*mSpecailPerson)[point]->mName[t]);
							}
							if (size == 4)
							{
								//cout << "**|";
